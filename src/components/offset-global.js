 ...
 * 
 */
AFRAME.registerComponent("offset-global", {
  schema: {
    target: {
      type: "selector"
    },
  ...
  init() {
    ...
    this.updateOffsetGlobal = this.updateOffsetGlobal.bind(this);

    waitForDOMContentLoaded().then(() => {
      if (this.data.on) {
        this.el.sceneEl.addEventListener(this.data.on, this.updateOffsetGlobal);
      } else {
        this.updateOffsetGlobal();
      }
    });
  },

  updateOffsetGlobal: (function () {
    ...
    return function () {
        console.log("updateOffsetGlobal called");
      ...
      offsetVector.copy(this.data.pos.add(this.data.offset));
      ...
      if (this.data.selfDestruct) {
        if (this.data.on) {
          this.el.sceneEl.removeEventListener(this.data.on, this.updateOffsetGlobal);
        }
        this.el.removeAttribute("offset-global");
      }
    };
  })()
});
